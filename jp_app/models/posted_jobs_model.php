...
class Posted_Jobs_Model extends CI_Model {
	...
	public function add_posted_job($data){
            ...
            $return = $this->db->insert('pp_post_jobs', $data);
            if ((bool) $return === TRUE) {
	...
	public function update_posted_job($id, $data){
		$this->db->where('ID', $id);
		$return=$this->db->update('pp_post_jobs', $data);
		return $return;
	}
	
	public function delete_posted_job($id){
		$this->db->where('ID', $id);
		$this->db->delete('pp_post_jobs');
	}
	...
	public function delete_posted_job_by_id_emp_id($id,$emp_id){
		$this->db->where('ID', $id);
		$this->db->where('employer_ID', $emp_id);
		$return = $this->db->delete('pp_post_jobs');
		return $return;
	}
	...
	public function get_posted_job_by_job_id($job_id) {
        $this->db->select('pp_post_jobs.*, pp_employers.email, pp_employers.first_name');
        $this->db->from('pp_post_jobs');
		$this->db->join('pp_employers', 'pp_post_jobs.employer_ID = pp_employers.ID', 'inner');
        $this->db->where('pp_post_jobs.ID', $job_id);
		$Q = $this->db->get();
        if ($Q->num_rows > 0) {
            $return = $Q->row();
        } else {
	...
	public function get_matching_searched_jobs($param, $per_page, $page) {
       $Q = $this->db->query("
	SELECT pj.ID, pj.job_title, pj.job_slug, pj.employer_ID, pj.company_ID, pj.job_description, pj.city, pj.dated, pj.last_date, pj.is_featured, pj.sts, pj.ip_address,pc.company_name, pc.company_logo, pc.company_slug
